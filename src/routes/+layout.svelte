<script lang="ts">
  ...
  import { UserCircleSolid, GridOutline, GridSolid, DrawSquareOutline, MapPinAltSolid, FileImageOutline } from 'flowbite-svelte-icons';
...
<div class="max-h-screen h-full overflow-hidden flex flex-col">
  <Tabs style="underline" contentClass=" " class="justify-end bg-gray-500/20">
    <TabItem open={$page.url.pathname === '/'} title="Profile" on:click={ () => loadPage('/') }>
      <div slot="title" class="flex items-center gap-2">
        <!-- <DrawSquareOutline size="sm" /> -->
        <MapPinAltSolid size="sm" />Kort
      </div>
    </TabItem>
    <TabItem open={$page.url.pathname === '/gallery'} title="Profile" on:click={() => loadPage('/gallery') }>
      <div slot="title" class="flex items-center gap-2">
  ...
  {#if loading}
    <div>
      <div class="absolute z-50 left-0 right-0 top-0 bottom-0 flex justify-center items-center" transition:fade >
        <Spinner size="36" class="absolute z-50"/>
      </div>
      <div class="relative">
        <div class="absolute z-40 inset-0 min-h-screen min-w-screen bg-[#f0efea]"  transition:fade></div>
      </div>
    </div>
  {/if}
